...
const PasswordOlvidada = () => {
  ...
  const handleSubmit = async e => {
    ...
    if (email === '' || email.length < 6) {
      setAlerta({
        mensaje: 'El email es obligatorio', error: true
      })
      return
    }
    try {
      const { data } = await clienteAxios.post('/usuarios/password-olvidada', { email })
      console.log(data)
      setAlerta({
        mensaje: data.msg
      })
  ...
  return (
    <>
      <div>
        <h1 className='text-indigo-600 font-black text-6xl'>Restablece tu contraseña</h1>
      </div>
